...
<script>
    var rowsCount = <?php echo json_encode($room->resources->count())?>;
                    ...
                    <form method="POST" action="{{route('roomUpdate', $room->id)}}" enctype="multipart/form-data">
                        ...
                        <fieldset>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" min="0" max="9999" id="occupancy" class="form-control @error('occupancy') is-invalid @enderror" name="occupancy" value="{{$room->occupancy}}" required>
                                @error('occupancy')
                                    <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                <div class="form-check form-switch">
                                    ...
                                    <label class="form-check-label" for="is_closed">Close Room</label>
                                </div>
                            </div>
                            @else<div class="mb-3">
                                <div class="form-check form-switch">
                            ...
                            <div class="row justify-content-center mb-3" id="resourcesList">
                                <table id="addResourcesTbl" class="table table-light table-borderless">
                                    ...
                                    <tbody id="addResourcesBody">
                                        @if (!$room->resources->count() < 1)
                                            @foreach ($room->resources as $index => $resource)
                                                <tr>
                                                    <td class="col-md-3">
                                                        <select name="resource_ids[]" class="form-select text-md-start">
                                                    ...
                                                    <td class="col-md-6">
                                                        <input type="text" name="descriptions[]" class="form-control" placeholder="(Optional) Enter a description" maxlength="255" minlength="0" value="{{$resource->pivot->description}}">
                                                    </td>
                                                    <td class="col-md-1 text-center">
                                                        <button type="button" class="btn btn-danger" onclick="deleteResource(this)"><i class="bi bi-trash3-fill"></i></button>
                                                    </td>
